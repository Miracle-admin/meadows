...
function validateForm(f){
	...
	var filecount = $$('input[name="serviceFiles[]"]').length;
	var minBasePrice = parseInt('<?php echo $minBasePrice; ?>');

	//validate price
	if($('price').get('value') < minBasePrice){
	...
	if($('extra-fast-enabled').checked){
		if($('extra-fast-duration').get('value').toInt() >= $('duration').get('value').toInt()){
			alert('<?php echo JText::_('COM_JBLANCE_FAST_DELIVERY_CANNOT_GREATER_THAN_EQUAL_TO_BASE_DURATION', true); ?>');
			$('extra-fast-duration').focus();
			return false;
		}
	}
...
window.addEvent('domready', function(){
	$$('input.service-extra-checkbox').each(function(el){
		el.addEvent('click', processExtraFields);
	});
	$$('input.service-extra-checkbox').fireEvent('click');
});

var processExtraFields = function(){
	var row = $(this).get('data-extra-row');	//get the row name or number
	if(this.checked)
		$$('input[type="text"][data-extra-row="'+ row +'"]').addClass('required').setProperty('required', 'required');
	else
		$$('input[type="text"][data-extra-row="'+ row +'"]').removeClass('required').removeProperty('required');
...
<div class="mtb-40" id="edit-service-wrapper">
<form action="<?php echo JRoute::_('index.php'); ?>" method="post" name="userFormProject" id="userFormProject" class="form-validate form-inline" onsubmit="return validateForm(this);" enctype="multipart/form-data" novalidate>
  ...
  <fieldset>
    ...
    <div class="control-group">
      ...
      <div class="controls">
        <textarea name="description" id="description" class="input-xxlarge required hasTooltip" rows="5" columns="50" title="<?php echo JHtml::tooltipText(JText::_('COM_JBLANCE_TT_SERVICE_DESCTIPTION')); ?>"><?php echo $row->description; ?></textarea>
      </div>
    </div>
  </fieldset>
  <fieldset>
    ...
    <div class="control-group">
      ...
      <div class="controls">
        <div class="input-prepend"> <span class="add-on"><?php echo $currencysym; ?></span>
    ...
    <div class="control-group">
      ...
      <div class="controls">
        <div class="input-append">
          ...
          <span class="add-on"><?php echo JText::_('COM_JBLANCE_BID_DAYS'); ?></span> </div>
      </div>
    </div>
  </fieldset>
  <fieldset>
    <legend><?php echo JText::_('COM_JBLANCE_SERVICE_IMAGES'); ?></legend>
    <div class="upload-dropzone" id="drop-zone"> <?php echo JText::_('COM_JBLANCE_DRAG_DROP_FILES_HERE'); ?> </div>
    <div id="actions" class="row-fluid">
      <div class="span7"> 
        
        <!-- The fileinput-button span is used to style the file input field as button --> 
        <span class="btn btn-success fileinput-button dz-clickable"> <i class="icon-plus icon-white"></i> <span><?php echo JText::_('COM_JBLANCE_ADD_FILES'); ?></span> </span>
        <button type="button" class="btn btn-primary start"> <i class="icon-upload icon-white"></i> <span><?php echo JText::_('COM_JBLANCE_START_UPLOAD'); ?></span> </button>
        <button type="reset" class="btn btn-warning cancel"> <i class="icon-ban-circle icon-white"></i> <span><?php echo JText::_('COM_JBLANCE_CANCEL_UPLOAD'); ?></span> </button>
      </div>
      <div class="span5"> 
    ...
    <div id="template" class="file-row"> 
      ...
      <div> <span class="preview"><img class="img-polaroid" data-dz-thumbnail /> </span> </div>
      <div>
      ...
      <div>
        <button type="button" class="btn btn-primary start"> <i class="icon-upload icon-white"></i> <span><?php echo JText::_('COM_JBLANCE_START'); ?></span> </button>
        <button data-dz-remove class="btn btn-warning cancel"> <i class="icon-ban-circle icon-white"></i> <span><?php echo JText::_('COM_JBLANCE_CANCEL'); ?></span> </button>
        <button data-dz-remove class="btn btn-danger delete"> <i class="icon-trash icon-white"></i> <span><?php echo JText::_('COM_JBLANCE_DELETE'); ?></span> </button>
      </div>
    </div>
    </div>
  </fieldset>
  ...
  <fieldset>
      ...
      <div class="row-fluid">
        <div class="span6">
          <label class="checkbox">
            <input type="hidden" name="extras[fast][enabled]" value="0" />
            <!-- this is added when checkbox is not checked -->
            ...
            <span class="label label-warning"><?php echo JText::_('COM_JBLANCE_FAST_DELIVERY'); ?></span> <?php echo JText::_('COM_JBLANCE_FAST_DELIVERY_DESC'); ?> </label>
        </div>
        <div class="span6">
          <div class="input-prepend"> <span class="add-on"><?php echo $currencysym; ?></span>
    ...
    <div class="well well-small">
      <div class="row-fluid">
        <div class="span6">
          <label class="checkbox">
            <input type="hidden" name="extras[<?php echo $i; ?>][enabled]" value="0" />
            <!-- this is added when checkbox is not checked -->
  ...
  <div class="alert alert-info">
    ...
    <ul>
      ...
      <li> <?php echo JText::sprintf('COM_JBLANCE_CHARGE_PER_SERVICE_INFO', JblanceHelper::formatCurrency($chargePerService)); ?> </li>
      <?php endif; ?>
      <?php if($serviceFee > 0) : ?>
      <li> <?php echo JText::sprintf('COM_JBLANCE_SERVICE_FEE_INFO', $serviceFee); ?> </li>
      <?php endif; ?>
    </ul>
  </div>
  <?php } ?>
  ...
  <div class="alert alert-block">
    ...
    <?php echo JText::_('COM_JBLANCE_SERVICE_WILL_BE_REVIEWED_BY_ADMIN_BEFORE_PUBLISH'); ?> </div>
  <?php } ?>
  <div class="form-actions add-ser-btn">
