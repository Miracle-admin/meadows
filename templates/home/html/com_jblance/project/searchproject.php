...
$doc->addScript("components/com_jblance/js/timeago.js");
...
<!--
    jQuery(document).ready(function () {
        ...
        jQuery('#budget').on('slideStop', function () {
            this.form.submit();
        });
...
<form action="<?php echo $action; ?>" method="get" name="userFormJob" enctype="multipart/form-data">
    <div class="col-md-3 project-filter-lf">
        ...
        <div class="p-type" style="display: none">
            <h5><?php echo JText::_('COM_JBLANCE_PROJECT_TYPE'); ?></h5>
            <div class="selct-cate">
                <!--                <label class="checkbox inline" for="">
                                    <input onclick="this.form.submit()" type="checkbox" id="fixed" name="project_type[fixed]" value="COM_JBLANCE_FIXED" <?php
                ...
                <?php echo JText::_('COM_JBLANCE_FIXED'); ?> </label>
                                <label class="checkbox inline"> 
                                   <input type="hidden" name="project_type['hourly']" value="0" /> 
                                    <input type="checkbox" onclick="this.form.submit()" id="hourly" name="project_type[hourly]" value="COM_JBLANCE_HOURLY" <?php
                if (isset($proj_type['hourly'])) {
                    echo 'checked';
                }
                ?> >
                <?php echo JText::_('COM_JBLANCE_HOURLY'); ?> </label>-->
                <input type="hidden" name="project_type[fixed]" value="COM_JBLANCE_FIXED" />
            </div>
        </div>
        <div class="p-status">
    ...
    <div class="col-md-9"> 
        ...
        <div class="npo-filter">
            ...
            <div class="fiter-new-old">
                <?php
                $list_order = $select->getRadioSearchOrder('order', $order);
                ?>
            </div>
        </div>
                    ...
                    for ($i = 0, $x = count($this->rows); $i < $x; $i++) {
                        ...
                        <div class="row projects-detail-wrapper">
                            ...
                            <div class="col-md-11 project-detail">
                                ...
                                <span class="budget">Budget $<?php echo $row->budgetmin . ' - $' . $row->budgetmax; ?></span>
                                <ul class="promotions">
                                ...
                                <p><?php
                                    ...
                                    echo (strlen($row->description) > 200) ? substr($row->description, 0, 450) . '...' : $row->description;
                                    ;
                                    ?></p>
                                ...
                                <p> <b>Posted: </b>  <time class="timeago"  title="<?php echo $startdate; ?>"></time></p>  
                                <!--<ul class="bid-footer">
                                <li></li>-->
                                <?php /* ?><li>Total Bid
                                ...
                                <?php /* ?>              <li><?php echo JText::_('COM_JBLANCE_STATUS'); ?><sub class="<?php echo $statusLabel; ?>"><?php echo JText::_($row->status); ?></sub></li><?php */ ?>
                                <!-- </ul>-->
                                ...
                                $user = JFactory::getUser();
                                // echo '<pre>'; print_r($row); die;
                                if (!JblanceHelper::getBid($row->id)) {
                                    if ($row->publisher_userid == $user->id) {
                                        ?>
                                        <div class="clearfix ">
                                            <a target="_blank" href="<?php echo JRoute::_("index.php?option=com_jblance&view=project&layout=projectdashboard&pid=" . $row->id . "&Itemid=340") ?>" class="abtn">See Progress</a>
                                        </div>
        <?php }
    } elseif (in_array("13", $user->groups)) { ?>
                                    <div class="clearfix ">
                                        <a target="_blank" href="<?php echo JRoute::_("index.php?option=com_jblance&view=project&layout=placebid&pid=$row->id&Itemid=197") ?>" class="abtn">Bid Now</a>
                                    </div>
    <?php } elseif (in_array("11", $user->groups)) {
        if ($row->publisher_userid == $user->id) {
            ?>
                                        <div class="clearfix ">
                                            <a target="_blank" href="<?php echo JRoute::_("index.php?option=com_jblance&view=project&layout=projectdashboard&pid=" . $row->id . "&Itemid=340") ?>" class="abtn">See Progress</a>
