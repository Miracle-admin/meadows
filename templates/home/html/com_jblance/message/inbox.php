    ...
    <div class="all-inbox-msh">
      <table class="table table-hover table-condensed">
		...
		if(count($this->msgs) == 0){		//Called if there are no messages -> Shows a text that spreads over the whole table
			?>
          <tr>
            <td colspan='5' align="center"><?php echo JText::_("COM_JBLANCE_NO_MESSAGES"); ?></td>
          </tr>
          <?php
		}
		$k = 0;
		...
		?>
          <tr id="jbl_feed_item_<?php echo $msg->id; ?>">
            ...
            <td><span id="feed_hide_<?php echo $msg->id; ?>" class="help-inline"> <a class="btn btn-mini" onclick="processMessage('<?php echo $msg->id; ?>', 'message.processmessage');" title="<?php echo JText::_('COM_JBLANCE_REMOVE'); ?>" href="javascript:void(0);"><i class="icon-remove"></i></a> </span></td>
          </tr>
          <?php 
			$k = 1 - $k;
		}
      ...
      <div class="pull-left"><a href="<?php echo $link_compose; ?>" class="btn btn-primary"><span><?php echo JText::_('COM_JBLANCE_COMPOSE'); ?></span></a></div>
    </div>
  </form>
</div>
