...
<form action="<?php echo JRoute::_('index.php'); ?>" method="post" name="profilePicture" class="form-validate edit-user-pic-wrapper mtb-40" onsubmit="return validateForm(this);">
  ...
  <?php 
	$tipmsg = JHtml::tooltipText(JText::_('COM_JBLANCE_ATTACH_IMAGE'), JText::_('COM_JBLANCE_ALLOWED_FILE_TYPES').' : '.$config->imgFileText.'<br>'.JText::_('COM_JBLANCE_MAXIMUM_FILE_SIZE').' : '.$config->imgMaxsize.' kB');
	?>
...
<form action="<?php echo JRoute::_('index.php'); ?>" method="post" name="profileThumbnail" class="form-validate edit-thumb-wrpper" onsubmit="return validateForm(this);">
  ...
  <div class="pull-left">
    ...
    <p> <a href="javascript:updateThumbnail('user.croppicture')" id="update-thumbnail"><?php echo JText::_('COM_JBLANCE_EDIT_THUMBNAIL'); ?></a> </p>
    <?php endif; ?>
  </div>
  
  <!-- show the edit thumbnail if the user has attached any picture -->
  
  <div id="editthumb" style="position: relative; top: 130px; margin-bottom: 180px; left: -10px; display:none;">
    <?php if($this->row->picture) : ?>
    <?php 
		//get image size
    ...
    <div id="imgouter">
      ...
      <div id="imglayer" style="width: <?=$width; ?>px; height: <?=$height ?>px; background-image: url('<?php echo JBPROFILE_PIC_URL.$this->row->picture?>')"> </div>
    </div>
