...
class ModJblanceDevdashboardLatestProjectsHelper {	
	
	public static function getLatestProjects($total_row,$pjStatus){
	$db	  = JFactory::getDbo();
	$user = JFactory::getUser();
	$now  = JFactory::getDate();
	$Pjstat=$pjStatus!="any"?"WHERE p.status=".$db->quote($pjStatus)." AND":"WHERE";
	$query = "SELECT p.*,(TO_DAYS(p.start_date) - TO_DAYS(NOW())) AS daydiff FROM #__jblance_project p ".$Pjstat." p.approved=1   AND '$now' > p.start_date ".
	...
	$rows2 = $db->loadObjectList();
		...
		if(count($rows2)){
			...
			foreach($rows2 as $row){
				...
				{
				$rows[$i] = new stdClass();
				$row->project_title = self::limitTitle($row->project_title, 50);
				$row->description=self::limitDescription($row->description, 200);
