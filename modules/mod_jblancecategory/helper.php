...
class ModJblanceCategoryHelper {	
	...
	public static function getSubCategories($id_category, $show_empty_count, $init, $indent){		
		...
		if($show_empty_count == 1){
			$query = "SELECT c.*, (SELECT count(p.id) FROM #__jblance_project p WHERE FIND_IN_SET(c.id, p.id_category) AND p.status='COM_JBLANCE_OPEN' AND p.approved=1 AND '$now' > p.start_date) AS thecount ".
