 ...
 */
class VirtueMartViewVmproducts extends VmView {
    ...
    public function display($tpl = null) {
        ...
        if (!VmConfig::get('shop_is_offline', 0)) {
            ...
            // $productModel->getProductListing('', $topTen_products_count);
            ...
            if (JRequest::getVar("group_id"))
                $group = JRequest::getVar("group_id");
            $subcatid = vRequest::getVar("sub_cat");
            $fmcatid = vRequest::getVar("fm_cat");
            $this->categoryId = vRequest::getVar('virtuemart_category_id', -1);
            if($fmcatid){
                $this->categoryId = $this->categoryId.','.$fmcatid;
            }
            if($subcatid){
                $this->categoryId = $this->categoryId.','.$subcatid;
            }
//echo $this->categoryId; die;
            $ids = $productModel->sortSearchListQuery(TRUE, $this->categoryId, $group);


            $this->perRow = empty($category->products_per_row) ? VmConfig::get('products_per_row', 3) : $category->products_per_row;
            $perRow = $this->perRow; //empty($category->products_per_row) ? VmConfig::get('products_per_row', 3) : $category->products_per_row;
            $this->assignRef('perRow', $perRow);
            $pagination = $productModel->getPagination($perRow);
            $this->assignRef('vmPagination', $pagination);

//echo $this->perRow; die;

            $this->products[$group] = $productModel->getProducts($ids); //getProductListing('', '12','','','','',$this->categoryId);
            
            $productModel->addImages($this->products[$group], 1);
