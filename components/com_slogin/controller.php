...
//1.Check Username OR Email
	function checkFacebookEmail($username)
	{	
		// Get a db connection.
		$db = JFactory::getDbo();
	...
	function facebookLogin(&$data)
	{
		...
		$query->where(  $db->quoteName('slogin_id') . ' = '. $db->quote($data['slogin_id']));			 
		// Reset the query using our newly populated query object.
		
		$db->setQuery($query);		 
		// Load the results as a list of stdClass objects (see later for more options on retrieving data).
		$resultInfo = $db->loadObject();
		return $resultInfo;	
	}
 /**
    ...
    public function sredirect()
    {
		...
		$sessionMenuItem =  $session->get('menuItem');
	
		$state = $app->getUserState('com_slogin.provider.info');
		 
		//echo "<pre>"; print_r($state); die;
		
		if(isset($state->first_name) AND !empty($state->first_name))
		{
			$username = $state->first_name.'.'.$state->last_name;
		}
		else if(isset($state->given_name) AND !empty($state->given_name))
		{
			$username = $state->given_name.'-'.$state->family_name;
		}
		
		$resultInfo = $this->checkFacebookEmail($username);
		
		if( (isset($resultInfo) AND !empty($resultInfo))  )
		{	
			$count = $resultInfo[0];			
			$count = $count->emailCount;
		}
		
		//echo "count - "; print_r($count); die;	
		
		if( (isset($state->id) AND !empty($state->id))  )
		{	
			$session->set('slogin_id', $state->id);			
		}

		 //load the plugin to validate facebook login	
		 if( (isset($state->email) AND !empty($state->email)) AND $user->id === 0) 
		 {
			//$username = $state->first_name.'.'.$state->last_name;
			$slogin_id = $state->id;
			
			//echo "<pre>username - ". $username; echo "<br/>count - ";  print_r($user); echo "<br>state"; print_r($state); print_r($count); die;		
			
			if($count == 1)
			{
				$credentials = Array('username' => $username, 'slogin_id' => $slogin_id);
				$return = $this->facebookLogin($credentials);	
				if($return->userCount == 1 AND $user->id === 0)		
				{	
					$options['action'] = 'core.login.site';	
					$userTemp = JFactory::getUser($credentials['username']);	
					$plgUser['user'] = $userTemp;
					...
					$block = $userTemp->block;
					$sendEmail = $userTemp->sendEmail;
					$registerDate = $userTemp->registerDate;
					$lastvisitDate = $userTemp->lastvisitDate;
					$activation = $userTemp->activation;
					$params = $userTemp->params; 
					$groups = $userTemp->groups;
					$slogin_id = $userTemp->slogin_id;
					$emailvalid = $userTemp->emailvalid;
					
					$param=array("id"=>$id,"name"=>$name,"username"=>$username,"email"=>$email,"password"=>$password,"block"=>$block,"sendEmail"=>$sendEmail,"registerDate"=>$registerDate,"lastvisitDate"=>$lastvisitDate,"activation"=>$activation,"params"=>$params,"groups"=>$groups, "slogin_id" => $slogin_id, "emailvalid" =>$emailvalid);
							
					//print_r($param); die;		
					
					$pluginInfo = JPluginHelper::importPlugin('user', 'fglogin');	
					$dispatcher =JDispatcher::getInstance();			
					$response= $dispatcher->trigger('onUserLogin', array(&$param, $options));	 			

					if((isset($response['0']) AND $response[0] == 1))
					{  
						$plugin = JPluginHelper::importPlugin('user', 'jblance');	
						$dispatch =JDispatcher::getInstance();			
						$response= $dispatch->trigger('onUserAfterLogin', array(&$plgUser));
					}
				}
				else
				{
					if(isset($state->first_name) AND !empty($state->first_name))
					{
						...
						$app->setUserState('email',$state->email);	
						$app->setUserState('password', $pwd);
						$app->setUserState('password2',$pwd);
						//$session->set('slogin_id', $state->id);						
					}
					else if(isset($state->given_name) AND !empty($state->given_name))
					{
					...
					$link = JRoute::_(JURI::root().'index.php?option=com_jblance&view=developer&layout=register&Itemid=347');
					$app->redirect($link, 'You must register as a developer first.', 'message');
				}
			}			
		 }
		...
		{		
			...
			//print_r($sessionMenuItem); die('login');
			if(isset($sessionMenuItem) AND $sessionMenuItem == 'login')	
			{
				$app->redirect($link, 'You must register as a developer first.', 'message');
			}
			else
    ...
    public function check_mail()
    {
        ...
        if(!$validator->validateEmail($email)){
			...
			$this->displayRedirect('index.php?option=com_jblance&view=developer&layout=register', false, $msg, 'error');
        }
    ...
    protected function storeOrLogin($popup=false)
    {
        ...
        if (!$sloginUserId){
            ...
            if($this->config->get('enable_edit_reg_fields', 0) && !$reg_fields_edited){
				...
				$this->displayRedirect('index.php?option=com_jblance&view=developer&layout=register', $popup, $msg);
            }
            ...
            if($this->config->get('query_email', 0)==1)
            {
                ...
                if(!$validator->validateEmail($this->email)){
                    ...
                    if($this->config->get('collate_users', 0) == 1)
                    {
						...
						$this->displayRedirect('index.php?option=com_jblance&view=developer&layout=register', $popup, $msg, 'error');
                    }
                    else
                    {
                        //$this->displayRedirect('index.php?option=com_slogin&view=mail', $popup, $msg, 'error');
