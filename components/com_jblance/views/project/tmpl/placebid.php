...
function validateForm(f){
	...
	if(valid == true){
		...
		if(project_type == 'COM_JBLANCE_HOURLY'){
			var cmt_prd = $('commitment_period').get('value');
			var cmt_int = $('commitment_interval').get('value');
			var delivery = $('delivery').get('value');
		...
		$('submitbtn').set('value', '<?php echo JText::_('COM_JBLANCE_SAVING'); ?>');
...
<form action="<?php echo JRoute::_('index.php'); ?>" method="post" name="userFormBid" id="userFormBid" class="form-validate form-horizontal" onsubmit="return validateForm(this);" enctype="multipart/form-data">
	...
	<div class="jbl_h3title sign-nda"><?php echo JText::_('COM_JBLANCE_SIGN_NDA'); ?></div>
	<div class="well well-small jbbox-gradient">
		<?php 
		$ndaFile  =  JURI::root().'components/com_jblance/images/nda.txt'; echo JText::sprintf('COM_JBLANCE_NDA_BID_MUST_AGREE', $ndaFile); ?> <label class="checkbox"><input type="checkbox" name="is_nda_signed" id="is_nda_signed" value="1" /><?php echo JText::_('COM_JBLANCE_NDA_FREELANCER_AGREE_TO_NDA'); ?></label>
	</div>
