...
$link_edit_project = JRoute::_('index.php?option=com_jblance&view=project&layout=editprojectcustom&pid=' . $row->id);
    ...
    window.addEvent('domready', function () {
        $('commentForm').addEvent('submit', function (e) {
            ...
            var req = new Request.HTML({
                ...
                data: $('commentForm'),
                onRequest: function () {
                    $('btnSendMessage').set({'disabled': true, 'value': '<?php echo JText::_('COM_JBLANCE_SENDING'); ?>'});
                },
                onSuccess: function (tree, response) {

                    var li = new Element('li');
                    var span = new Element('span', {'text': response[1].get('text')}).inject(li);
                    var span1 = new Element('span', {'text': '<?php echo JText::_('COM_JBLANCE_RECENTLY'); ?>', 'class': 'pull-right'}).inject(span);
...
for ($i = 0, $x = count($this->forums); $i < $x; $i++) {
    ...
    ?>
          <li> <span><?php echo LinkHelper::GetProfileLink($forum->user_id, $poster); ?> <span class="pull-right"> <?php echo JblanceHelper::showTimePastDHM($postDate, 'SHORT'); ?> </span> </span>
...
<div class="col-md-4 trs-none">
  <div class=" shopper-info ">
    <div class="media">
      ...
      <div class="col-md-5">
        ...
        <h4> <?php echo $this->escape($publisher->$nameOrUsername); ?> </h4>
    ...
    <div class="row-fluid">
      <div class=" pro-attr">
        <ul>
          ...
          <li>
            <div class="row-t"><?php echo JText::_('COM_JBLANCE_AVG_BID').' ('.$currencycode.')'; ?> </div>
            <div class="row-b">
              <?php
							$projHelper = JblanceHelper::get('helper.project');		// create an instance of the class ProjectHelper
  ...
  <div class="bid-on-wrap"> 
    <!-- show the bid button only if the status is OPEN & not expired -->
    <?php if($row->status == 'COM_JBLANCE_OPEN' && !$isMine) : ?>
    <?php $link_place_bid = JRoute::_( 'index.php?option=com_jblance&view=project&layout=placebid&pid='.$row->id); ?>
    <a href="<?php echo $link_place_bid; ?>" class="abtn"><?php echo JText::_('COM_JBLANCE_BID_ON_THIS_PROJECT'); ?></a>
    <?php endif; ?>
  </div>
