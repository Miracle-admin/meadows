...
class JblanceControllerUser extends JControllerLegacy {
    ...
    function saveProfile() {
        ...
        for ($i = 1; $i <= 31; $i++) {
          ...
          //  if (JRequest::getVar($field, '') != '') {
                $extraFields[$field] = JRequest::getVar($field, '');
                $app->setUserState($field, JRequest::getVar($field, ''));
           // }
        }
        ...
        $dispatcher->trigger('onUserSaveProfile', array($jbuser, false, $extraFields));

        $Itemid = JRequest::getInt("Itemid");
        $link = JRoute::_('index.php?option=com_jblance&view=user&layout=dashboard&Itemid=394', false);
    ...
    function uploadPicture() {
        ...
        JBMediaHelper::uploadProfilePictureMedia();
    }
    ...
    function favourite() {
        ...
        if ($action == 1)
            $query = "INSERT INTO #__jblance_favourite(`actor`,`target`,`type`) VALUES('$user->id', '$targetId', 'profile')";
        else
