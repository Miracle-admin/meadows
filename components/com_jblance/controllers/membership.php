...
class JblanceControllerMembership extends JControllerLegacy {
	...
	function upgradeSubscription(){
		...
		if((isset($post['plan_id']) OR empty($post['plan_id'])) AND (isset($_REQUEST['plan_id']) AND !empty($_REQUEST['plan_id'])))
		{
		$post['plan_id'] = $_REQUEST['plan_id'];
		}
		...
		if($type == 'COM_JBLANCE_PROJECT'){
			...
			if($project->project_type == 'COM_JBLANCE_FIXED'){
				...
				if($sum == $bidamount){
					...
					$fee_from_buyer  = $projHelp->calculateProjectFee($project->publisher_userid, $bid->amount, 'buyer', 'COM_JBLANCE_FIXED');
					...
					$fee_from_lancer = $projHelp->calculateProjectFee($project->assigned_userid, $bid->amount, 'freelancer', 'COM_JBLANCE_FIXED');
					$transDtl = JText::sprintf('COM_JBLANCE_PROJECT_COMMISSION_FOR_PROJECT_NAME', $project->project_title);
	...
	$app  	  = JFactory::getApplication();
	$user	= JFactory::getUser();
	$planId=JRequest::getInt('id'); 
	/* $plan	= JTable::getInstance('plan', 'Table');
	$plan->load($planId); */
	$session = JFactory::getSession();
	$id = $session->set('id', $planId, 'upgsubscr');
	
	
	if ($user->guest  === 0) 
	{
		//echo $planId; print_r($_REQUEST); die;
		//$link = JRoute::_('index.php?option=com_jblance&view=membership&layout=check_out', false);		
		$link = JRoute::_('index.php?option=com_jblance&task=membership.upgradeSubscription&plan_id='.$planId."&".JSession::getFormToken()."=1", false);			
	
		$msg = JTEXT::_('COM_JBLANCE_PROCEED_PAYMENT_AFTER_REGISTRATION');
	...
	{	
		//echo "register "; die;
		$link = JRoute::_('index.php?option=com_jblance&view=developer&layout=register&plan_id='.$planId."&".JSession::getFormToken()."=1", false);				
		$app->redirect($link);
		return true;
	}
