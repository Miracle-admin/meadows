...
class JblanceModelProject extends JModelLegacy {
    ...
    function getPlaceBid() {
        ...
        $db = JFactory::getDbo();
        $user = JFactory::getUser();
        $id = $app->input->get('pid', 0, 'int'); //id is the "project id"
        $finance = JblanceHelper::get('helper.finance');  // create an instance of the class FinanceHelper
        ...
        $planStatus =  JblanceHelper::getBtPlan($user->id);//JblanceHelper::planStatus($user->id);
      //  echo '<pre>'; print_r($planStatus); die;
        if (empty($bid_id) && empty($planStatus) && ($planStatus != "active")) {
            $msg = JText::sprintf('COM_JBLANCE_NOT_ALLOWED_TO_DO_OPERATION_NO_ACTIVE_SUBSCRIPTION');
            $link = JRoute::_('index.php?option=com_jblance&view=user&layout=dashboard', false);
            $app->enqueueMessage($msg, 'error');
            $app->redirect($link);
    ...
    function getProjectProgress() {
        ...
        $bidid = $app->input->get('id', 0, 'int'); //bid id

        $isOwnedOperation = JblanceHelper::checkOwnershipOfOperation($pid, 'projectprogress'); //check ownership
        ...
        $query = "SELECT p.id project_id, p.publisher_userid buyer_id, p.assigned_userid freelancer_id, p.project_title, " .
                ...
                " WHERE b.id=" . $db->quote($bidid); //echo $query;
        $db->setQuery($query);
        $row = $db->loadObject();
...
//echo '<pre>'; print_r($rows); die;
    ...
    function getInviteUser() {
        ...
        $project_id = $app->input->get('pid', 0, 'int');
        $project = JTable::getInstance('project', 'Table');
   ...
   function getUpgradeProjectPlan() {
        $app = JFactory::getApplication();
        $db = JFactory::getDbo();
        
                $query = "SELECT * FROM `#__jblance_plan` where `id`='5'";
                $db->setQuery($query);
                $UpgradeProject = $db->loadObjectList();
        return $UpgradeProject;
    }
    //individual project dashboard

    function getProjectdashboard($pid = null) {
        $app = JFactory::getApplication();
        $db = JFactory::getDbo();
        $user = JFactory::getUser();
        //$id = $app->input->get('id', 0, 'int');
        $id = $pid = ($pid) ? $pid : $app->input->get('pid', 0, 'int');
        ...
        $status = $pj->status;
        $freelancer = $pj->publisher_userid == $user->id ? true : false;

//echo $pj->publisher_userid ; echo $user->id; die;
        if ($id > 0) {
            ...
            if ($apr == 1 && $user->emailvalid == '') {
                ...
                $query = "SELECT * FROM #__jblance_bid WHERE project_id=" . $db->quote($id) . " AND status='COM_JBLANCE_ACCEPTED'";
                $db->setQuery($query);
                $bid = $db->loadAssoc();
                $bId = $bid['id'];
            }
...
//            die;
    ...
    function upsellings($val) {
        echo '<pre>';
    print_r($val);
    $arrayName = array('id' =>'393');
    $row = JTable::getInstance('project', 'Table',$arrayName);
    print_r($row);
    die('model');
    }
