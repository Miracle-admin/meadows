...
class PaypalSubscriptions implements IPaymentProcessor {
	...
	public function getHtmlFields() {	
		$html  = '';
		$html .= sprintf('<input type="hidden" name="cmd" id="cmd" value="_xclick-subscriptions"/>');
		...
		$html .= sprintf('<input type="hidden" name="txn_type" value="%s" />', $this->txn_type);
		$html .= sprintf('<input type="hidden" name="subscr_date" value="%s" />', $this->subscr_date);
		...
		$html .= sprintf('<input type="hidden" name="rm" value="2" />'); 
		
		$html .= sprintf('<input type="hidden" name="src" value="%d" />', $this->recurring);
		$html .= sprintf('<input type="hidden" name="srt" value="%d" />', $this->recur_times);
	
		return $html;
	}
	
	public function processTransaction($data){
		$this->returnUrl = JRoute::_('index.php?option=com_jbusinessdirectory&task=payment.processPaypalSubscriptionsResponse&processor=paypalsubscriptions',false,-1);
		$this->notifyUrl = JRoute::_('index.php?option=com_jbusinessdirectory&task=payment.processPaypalSubscriptionsAutomaticResponse&processor=paypalsubscriptions',false,-1);
		...
		$this->recurring = 1;
		$this->recur_times=12;
		$this->txn_type = "subscr_signup";
		$this->subscr_date = date("Y-m-d");
		$this->days = $data->package->days;
		$this->timeUnit = $data->package->time_unit;
		$this->timeAmount = $data->package->time_amount;
