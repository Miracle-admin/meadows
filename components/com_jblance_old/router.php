...
function JblanceBuildRoute(&$query){
	...
	if(in_array('viewprofile',$segments))
	{
	unset($segments[0]);
	unset($segments[1]);
	}
	
	//remove the alias if it is view profile link
	/* if($query['Itemid']==149)
	{
	unset( $query['Itemid'] );
	}
...
function JblanceParseRoute($segments){
	...
	*///$segments[0] would either be task or view. Switch case the tasks
	...
	{
	$uname=$segments[0];
	$query="SELECT id FROM #__users WHERE username='".$uname."'";
	$db->setQuery($query);
	$id=$db->loadObject();
	$id=$id->id;
