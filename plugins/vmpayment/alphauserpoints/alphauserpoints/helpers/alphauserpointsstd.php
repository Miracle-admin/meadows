...
//https://cms.alphauserpoints.com/mx/cgi-bin/?cmd=_render-content&content_ID=developer/e_howto_html_Appx_websitestandard_htmlvariables

class AlphauserpointsHelperAlphaUserpointsStd extends AlphauserpointsHelperAlphauserpoints {
	...
	function __construct($method, $alphauserpointsPlugin) {
		...
		if ($this->_method->sandbox) {
			...
			$this->merchant_email = $this->_method->alphauserpoints_merchant_email;
		}
		if (empty($this->merchant_email)) {
			...
			$text = vmText::sprintf('VMPAYMENT_ALPHAUSERPOINTS_PARAMETER_REQUIRED', vmText::_('VMPAYMENT_ALPHAUSERPOINTS_' . $sandbox . 'MERCHANT'), $this->_method->payment_name, $this->_method->virtuemart_paymentmethod_id);
			//vmError($text, $text);
			return true;
		}
	}
	...
	public function preparePost() {
		...
		switch ($this->_method->payment_type) {
			...
			case '_xclick-subscriptions':
				...
				$post_variables['sra'] = 1; //Reattempt on failure. If a recurring payment fails, AlphaUserpoints attempts to collect the payment two more times before canceling the subscription.
		...
		$url = $this->_getAlphaUserpointsUrl();
		if (vmconfig::get('css')) {
			$msg = vmText::_('VMPAYMENT_ALPHAUSERPOINTS_REDIRECT_MESSAGE', true);
		} else {
			$msg='';
		}
		...
		if ($this->_method->debug) {
			$html .= '<form action="' . $url . '" method="post" name="vm_alphauserpoints_form" target="alphauserpoints">';
		} else {
			$html .= '<form action="' . $url . '" method="post" name="vm_alphauserpoints_form" id="vmPaymentForm" accept-charset="UTF-8">';
		}
		...
		if ($this->_method->debug) {
			...
			$html .= '<div style="background-color:red;color:white;padding:10px;">
						<input type="submit"  value="The method is in debug mode. Click here to be redirected to AlphaUserpoints" />
						</div>';
			$this->debugLog($post_variables, 'AlphaUserpoints request:', 'debug');

		} else {
			$html .= '<input type="submit"  value="' . vmText::_('VMPAYMENT_ALPHAUSERPOINTS_REDIRECT_MESSAGE') . '" />';
	...
	function initPostVariables($payment_type) {
		...
		if ($payment_type == '_xclick') {
			$post_variables['address_override'] = $this->_method->address_override; // 0 ??   Alphauserpoints does not allow your country of residence to ship to the country you wish to
		}
...
// TODO Check that paramterer
	...
	function onShowOrderBEPaymentByFields($payment) {
		...
		foreach ($showOrderBEFields as $key => $showOrderBEField) {
			...
			if ($payment->$field) {
				$html .= $this->alphauserpointsPlugin->getHtmlRowBE($prefix . $key, $payment->$field);
			}
		}
