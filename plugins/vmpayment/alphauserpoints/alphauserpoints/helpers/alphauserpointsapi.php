...
class AlphauserpointsHelperAlphaUserpointsApi extends AlphauserpointsHelperAlphauserpoints {
	...
	function initPostVariables ($alphauserpointsMethod) {
		...
		//$post_variables['version']	 	= "106.0"; //https://developer.alphauserpoints.com/webapps/developer/docs/classic/release-notes/
	...
	function DoPayment () {
		...
		if ($this->handleResponse()) {
			...
			$this->response['alphauserpoints_response_txn_type'] = 'DoDirectPayment';
			return true;
		} else {
			return false;
		}
...
// shipment value must include tax
	...
	function DoCapture ($payment) {
		...
		if (strcasecmp($alphauserpoints_data->PAYMENTSTATUS, 'Pending') != 0 && strcasecmp($alphauserpoints_data->PENDINGREASON, 'authorization') != 0) {
			return false;
		}
		...
		$reauth = $this->doReauthorize($alphauserpoints_data->txn_id, $alphauserpoints_data);
		if ($reauth === false) {
			$post_variables['AuthorizationID'] = $alphauserpoints_data->TRANSACTIONID;
		} else {
	...
	function RefundTransaction ($payment) {
		...
		$alphauserpoints_data = json_decode($payment->alphauserpoints_fullresponse);
		if ($alphauserpoints_data->PAYMENTSTATUS == 'Completed') {
			$post_variables = $this->initPostVariables('RefundTransaction');
			$post_variables['REFUNDTYPE'] = 'Full';
		} else {
			if ($alphauserpoints_data->PAYMENTSTATUS == 'Pending' && $alphauserpoints_data->PENDINGREASON == 'authorization') {
				$post_variables = $this->initPostVariables('DoVoid');
			} else {
				vmInfo('VMPAYMENT_ALPHAUSERPOINTS_CANNOT_REFUND');
				return false;
			}
		}

		$post_variables['TRANSACTIONID'] = $alphauserpoints_data->TRANSACTIONID;
	...
	function validate ($enqueueMessage = true) {
		...
		if (!Creditcard::validate_credit_card_number($cc_type, $cc_number)) {
			$errormessages[] = 'VMPAYMENT_ALPHAUSERPOINTS_CC_CARD_NUMBER_INVALID';
			$cc_valid = false;
		}
		...
		if (!Creditcard::validate_credit_card_cvv($cc_type, $cc_cvv, $required)) {
			$errormessages[] = 'VMPAYMENT_ALPHAUSERPOINTS_CC_CARD_CVV_INVALID';
			$cc_valid = false;
		}
		if (!Creditcard::validate_credit_card_date($cc_type, $cc_expire_month, $cc_expire_year)) {
			$errormessages[] = 'VMPAYMENT_ALPHAUSERPOINTS_CC_CARD_DATE_INVALID';
			$cc_valid = false;
		}
		if (!$cc_valid) {
	...
	function displayExtraPluginInfo () {
		...
		if ($this->customerData->getVar('cc_number')) {
			...
			$creditCardInfos = '<br /><span class="vmpayment_cardinfo">' . vmText::_('VMPAYMENT_ALPHAUSERPOINTS_CC_CCTYPE') . $this->customerData->getVar('cc_type') . '<br />';
			$creditCardInfos .= vmText::_('VMPAYMENT_ALPHAUSERPOINTS_CC_CCNUM') . $cc_number . '<br />';
			$creditCardInfos .= vmText::_('VMPAYMENT_ALPHAUSERPOINTS_CC_CVV2') . '****' . '<br />';
			$creditCardInfos .= vmText::_('VMPAYMENT_ALPHAUSERPOINTS_CC_EXDATE') . $this->customerData->getVar('cc_expire_month') . '/' . $this->customerData->getVar('cc_expire_year');
			$creditCardInfos .= "</span>";
			$extraInfo .= $creditCardInfos;
		} else {
			//$extraInfo .= '<br/><a href="' . JRoute::_('index.php?option=com_virtuemart&view=cart&task=editpayment&Itemid=' . vRequest::getInt('Itemid'), false) . '">' . vmText::_('VMPAYMENT_ALPHAUSERPOINTS_CC_ENTER_INFO') . '</a>';
		}
