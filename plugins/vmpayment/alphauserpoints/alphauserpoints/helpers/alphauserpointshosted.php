...
/**
 *
 * Alphauserpoints  Hosted Pro payment plugin
 *
...
// https://cms.alphauserpoints.com/cms_content/GB/en_GB/files/developer/HostedSolution.pdf

class AlphauserpointsHelperAlphaUserpointsHosted extends AlphauserpointsHelperAlphauserpoints {

	// Pay Now button; since version 65.1
	const BM_BUTTON_TYPE = 'PAYMENT';
	//A secure button, not stored on AlphaUserpoints, used only to initiate the Hosted Solution checkout flow;
	//default for Pay Now button. Since version 65.1
	const BM_BUTTON_CODE = 'TOKEN';
	const BM_BUTTON_VERSION = '104.0';
	const ALPHAUSERPOINTS_USER_LG = 64;
	var $api_login_id = '';
	var $api_signature = '';
	var $api_password = '';
	...
	function __construct($method,$alphauserpointsPlugin) {
		...
		if ((empty ($this->_method->payflow_partner) OR empty($this->_method->sandbox_payflow_partner))) {
			...
			$text = vmText::sprintf('VMPAYMENT_ALPHAUSERPOINTS_PARAMETER_REQUIRED', vmText::_('VMPAYMENT_ALPHAUSERPOINTS_' . $sandbox . 'PAYFLOW_PARTNER'), $this->_method->payment_name, $this->_method->virtuemart_paymentmethod_id);
			vmError($text);
		}
	}
	...
	// todo check the alphauserpoints langauge: can it be sent. Atm sent in the country lanaguge
	...
	function initPostVariables($alphauserpointsMethod) {
		...
		$post_variables['VERSION'] = self::BM_BUTTON_VERSION; //https://developer.alphauserpoints.com/webapps/developer/docs/classic/release-notes/
		...
		$post_variables['BUTTONIMAGEURL'] = 'https://www.alphauserpoints.com/en_US/i/btn/btn_paynow_SM.gif'; //we automatically redirect to alphauserpoints
	...
	function addUrls(&$post_variables) {
		$post_variables['L_BUTTONVAR']['return'] = JURI::root().'index.php?option=com_virtuemart&view=pluginresponse&task=pluginresponsereceived&alphauserpointsproduct=hosted&on=' . $this->order['details']['BT']->order_number . '&pm=' . $this->order['details']['BT']->virtuemart_paymentmethod_id . '&Itemid=' . vRequest::getInt('Itemid').'&lang='.  vRequest::getCmd('lang','');
		...
		$post_variables['L_BUTTONVAR']['cancel_return'] =JURI::root().'index.php?option=com_virtuemart&view=pluginresponse&task=pluginUserPaymentCancel&alphauserpointsproduct=hosted&on=' . $this->order['details']['BT']->order_number . '&pm=' . $this->order['details']['BT']->virtuemart_paymentmethod_id . '&Itemid=' . vRequest::getInt('Itemid').'&lang='.  vRequest::getCmd('lang','');
	...
	function redirectToAlphaUserpoints() {
		...
		if ($this->_method->debug AND $this->_method->template != 'templateD') {
			echo '<div style="background-color:red;color:white;padding:10px;">The method is in debug mode. <a href="' . $emailink . '">Click here to be redirected to AlphaUserpoints</a></div>';
			echo '<div style="background-color:red;color:white;padding:10px;">The method is in debug mode. ' . $websitecode . 'Click here to be redirected to AlphaUserpoints</a></div>';
			jexit();
		} else {
	...
	function DoCapture($payment) {
		...
		if ($alphauserpoints_data->payment_status != 'Pending' && $alphauserpoints_data->pending_reason != 'Authorization') {
			return false;
		}
		$post_variables = $this->initPostVariables('DoCapture');

		//Do we need to reauthorize ?
		$reauth = $this->doReauthorize($payment->alphauserpoints_response_txn_id, $alphauserpoints_data);

		// the authorisation identification number of the payment you want to capture.
		if ($reauth === false) {
			$post_variables['AUTHORIZATIONID'] = $alphauserpoints_data->txn_id;
		} else {
...
// (Optional) Your invoice number or other identification number that is displayed to the merchant and customer in his transaction history.
	...
	/**
	 * https://developer.alphauserpoints.com/webapps/developer/docs/classic/alphauserpoints-payments-standard/integration-guide/authcapture/
	 * @param $AuthorizationID
	 * @param $alphauserpoints_data
	 * @return bool
	 */
	function doReauthorize($AuthorizationID, $alphauserpoints_data) {
        ...
        $post_variables['PAYERID'] = $alphauserpoints_data->payer_id; // Unique AlphaUserpoints customer account identification number
        $post_variables['AUTHORIZATIONID'] = $AuthorizationID;
        $post_variables['PAYMENTACTION'] = 'DoReauthorization';
        $post_variables['AMT'] =  $alphauserpoints_data->mc_gross; // ???
        $post_variables['CURRENCYCODE'] = $alphauserpoints_data->mc_currency;
	...
	function RefundTransaction($payment) {
		...
		$alphauserpoints_data = json_decode($payment->alphauserpoints_fullresponse);
		if (strcasecmp($alphauserpoints_data->payment_status, 'Completed') == 0) {
			$post_variables = $this->initPostVariables('RefundTransaction');
			$post_variables['REFUNDTYPE'] = 'Full';
			$post_variables['TRANSACTIONID'] = $alphauserpoints_data->txn_id;

		} else if (strcasecmp($alphauserpoints_data->payment_status, 'Pending') == 0 && strcasecmp($alphauserpoints_data->pending_reason, 'authorization') == 0) {
