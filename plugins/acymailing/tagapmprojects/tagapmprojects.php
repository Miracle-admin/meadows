...
class plgAcymailingTagapmprojects extends JPlugin
{

	function plgAcymailingApmprojects(&$subject, $config){
	 ...
	 function acymailing_getPluginType() {
		...
		//Tab name for the tag interface
	 	$onePlugin->name = 'Appmeadows Project Tags';
		...
		//The value for this variable should change for each plugin, don't forget to make it unique for your own plugin!! (this is the function name you will use just below)
	 	$onePlugin->function = 'acymailingtagapmprojects_show';
	...
	//If you don't want an interface on the tag system, just remove this function
	 function acymailingtagapmprojects_show(){
	 ?>
	 <script language="javascript" type="text/javascript">
			function applyTag(tagname){
				var string = '{apmptag:'+tagname;
	 ...
	 <?php
	    ...
	    $jbtags = (array)acymailing_get('helper.jbtags');
	...
	<p><b style='color:red;'>*tag is only available during user registration.</b></p><p><b><a style='color:green;' target='_blank' href='".JUri::base()."index.php?option=com_acymailing&ctrl=newsletter&task=viewContextualTags'>Click here</a></b> To view available tags in different contexts.</p></div>";
