...
class plgSearchVmvendors extends JPlugin
{
	...
	* @param string ordering option, newest|oldest|popular|alpha|category
	...
	function onContentSearch($text, $phrase='', $ordering='', $areas=null)
	{
		...
		$q ="SELECT vv.`vendor_name` AS title , vv.`created_on` AS created ,  vvl.`vendor_store_desc` AS text, `u`.id ,u.`emailvalid` , '".JText::_('PLG_SEARCH_VMVENDORS_VENDORS')."' AS section 
			...
			AND u.`block`='0' AND u.`emailvalid` ='' 
			ORDER BY ".$order;
		...
		if ($rows) {
			...
			{
				$rows[$key]->type = 'Vendors';
				$rows[$key]->href = 'index.php?option=com_jblance&view=user&layout=viewprofile&id='.$row->id.'&Itemid='.$vmvendor_itemid;
			}
		}
