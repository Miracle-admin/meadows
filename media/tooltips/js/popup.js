...
(function($) {
	...
	nnTooltipsPopup = {
		...
		insertText: function() {
			...
			var tip = this.getValue('tip_text');

			if (tip == '') {
				alert(window['tooltips_error_empty_' + tip_type]);
				return false;
			}
			...
			var link_type = this.getValue('link_type');
			if (link_type == 'image') {
				var link = this.getValue('link_image');

				if (link != '') {
					var attribs = 'src="' + link + '" ' + this.getValue('link_image_attribs');

					var link = '<img ' + attribs.trim() + ' />';
				}
			} else {
				var link = this.getValue('link_text');

			}

			if (link == '') {
				link = tooltips_text_placeholder;
			}
		...
		getValue: function(name) {
			return $('[name="tooltip\\[' + name + '\\]"]').val().trim();
		},
