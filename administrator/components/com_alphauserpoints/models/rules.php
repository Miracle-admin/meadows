...
class alphauserpointsModelRules extends JmodelLegacy {
	...
	function _delete_rule() {
		...
		JControllerLegacy::setRedirect('index.php?option=com_alphauserpoints&task=rules', $msg, $msgType);
	...
	function _save_rule($apply=0) {
		...
		if (!$apply){
			...
			JControllerLegacy::setRedirect('index.php?option=com_alphauserpoints&task=editrule&cid[]='.$id, $msg);
		}
		JControllerLegacy::redirect(); 				
