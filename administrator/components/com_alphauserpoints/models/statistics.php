...
class alphauserpointsModelStatistics extends JmodelLegacy {
	...
	function _load_users() {	
		...
		$filter_order		= $app->getUserStateFromRequest( "com_alphauserpoints.filter_order",		'filter_order',		'u.name',	'cmd' );
		$filter_order_Dir	= $app->getUserStateFromRequest( "com_alphauserpoints.filter_order_Dir",	'filter_order_Dir',	'',			'word' );
	...
	function _save_user($apply=0) {
		...
		if (!$apply){
			JControllerLegacy::setRedirect('index.php?option=com_alphauserpoints&task=statistics', $msg);
		} else {
			JControllerLegacy::setRedirect('index.php?option=com_alphauserpoints&task=edituser&cid[]='.$id. '', $msg);
		}
