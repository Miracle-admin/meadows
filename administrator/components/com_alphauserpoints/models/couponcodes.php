...
class alphauserpointsModelCouponcodes extends JmodelLegacy {
	...
	function _delete_coupon() {
		...
		JControllerLegacy::setRedirect('index.php?option=com_alphauserpoints&task=couponcodes', $msg, $msgType);
	...
	function _save_coupon($apply=0) {
		...
		if (!$apply){
			...
			JControllerLegacy::setRedirect('index.php?option=com_alphauserpoints&task=editcoupon&cid[]='.$id, $msg);
		}
		JControllerLegacy::redirect();
	}
