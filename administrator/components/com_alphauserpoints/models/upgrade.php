...
class alphauserpointsModelUpgrade extends JmodelLegacy {
	...
	function __construct(){
	 ...
	 function _getUpdate()
	 {
	 	$url = 'http://update.alphaplug.com/alphauserpoints_1.9_update.xml';
		$data = '';
		$check = array();
		$check['connect'] = 0;
		...
		if(function_exists('fsockopen') && $data == '') {
			...
			if ($fsock) {
				@fputs($fsock, "GET /alphauserpoints_1.9_update.xml HTTP/1.1\r\n");
		...
		if( $data && strstr($data, '<?xml version="1.0" encoding="utf-8"?><update>') ) {
			...
			$filepath     = JPATH_SITE . DS . 'tmp' . DS . 'checkaupversion.xml';
	 ...
	 function _checkNewRule()
	 {
		...
		if ( file_exists( $rule_dailylogin ) ) 
		{
			...
			{
				$query = $insertinto .
				"('', 'Login', 'Attrib or remove points when a user log in into the frontend website', 'Joomla user', 'sysplgaup_login', '1', '', '', '', 0, 0, 0, '0000-00-00 00:00:00', '', '', '', '', 0, 0, 0, 0, 1, 1, 'us', '1', '', '2', '0', '', '', '0', '0', '0', '0', '0');";
				$db->setQuery( $query );
				if ( $db->query() ) {		
					$nrule++;
				}
		...
		if ( file_exists( $rule_happybirthday ) ) 
		{
			...
			{
				$query = $insertinto . 
				"('', 'AUP_HAPPYBIRTHDAY', 'AUP_HAPPYBIRTHDAY_DESCRIPTION', 'Joomla user', 'sysplgaup_happybirthday', '1', '', '', '', 0, 0, 0, '0000-00-00 00:00:00', '', '', '', '', 0, 0, 0, 0, 1, 1, 'us', '1', '', '4', '0', '', '', '0', '0', '0', '0', '0');";
				$db->setQuery( $query );
				if ( $db->query() ) {		
					$nrule++;
				}
		...
		======================================== */
		$rule_submitnewarticle = JPATH_SITE.DS.'plugins'.DS.'content'.DS.'sysplgaup_submitarticle'.DS.'sysplgaup_submitarticle.php';
