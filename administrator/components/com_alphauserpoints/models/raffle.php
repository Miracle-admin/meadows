...
class alphauserpointsModelRaffle extends JmodelLegacy {
	...
	function _delete_raffle() {
		...
		JControllerLegacy::setRedirect('index.php?option=com_alphauserpoints&task=raffle', $msg, $msgType);
	...
	function _save_raffle($apply=0) {
		...
		if (!$apply){
			JControllerLegacy::setRedirect('index.php?option=com_alphauserpoints&task=raffle', $msg);
		} else {
			JControllerLegacy::setRedirect('index.php?option=com_alphauserpoints&task=editraffle&cid[]='.$id, $msg);
		}
		JControllerLegacy::redirect(); 
	}
