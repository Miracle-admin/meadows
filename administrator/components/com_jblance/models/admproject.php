...
class JblanceModelAdmproject extends JModelLegacy {
 	...
 	function getShowUser(){
		...
		$jinput = $app->input;
		$featured=$jinput->get('featured', 3, 'int');
		...
		if($featured!=3)
		{
		$where[]='ju.featured='.$db->quote($featured);
		}
 		...
 		$query = "SELECT u.*,ju.biz_name,ju.featured FROM #__users u".
 	...
 	function getShowsEscrow(){
 		...
 		$query = "SELECT e.*,s.name sender,r.name receiver,COALESCE(p.project_title, sv.service_title) title FROM #__jblance_escrow e ".
				 ...
				 "LEFT JOIN #__jblance_service sv ON (e.project_id = sv.id AND e.type = 'COM_JBLANCE_SERVICE') ".
	...
	function getDeveloperRecommend(){
		...
		//echo "<pre>";
		//print_r($row->id_category);		
		//$platforms = $app->input->getArray('id_category');
		
 		//make selection user
 		 $query = "SELECT u.id AS value, u.username, name FROM #__jblance_user ju ".
				 " LEFT JOIN #__users u ON ju.user_id=u.id ".
				 " WHERE ju.id_category IN ( ".$row->id_category. ") " .
				 " ORDER BY ju.user_id";
 		$db->setQuery($query);
 		$users = $db->loadObjectList();
		...
		//echo "<pre>"; print_r($users);
 		...
 		$lists['developerlist'] 	= JHtml::_('select.genericlist', $types, 'publisher_userid', 'class="input-xlarge advancedSelect" size="1" multiple="multiple"',  'value', 'text', $row->publisher_userid ? $row->publisher_userid : '0');
