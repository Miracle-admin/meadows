...
class JblanceModelAdmconfig extends JModelLegacy {
	...
	function getShowLocation(){
		...
		$limit			  = $app->getUserStateFromRequest('global.list.limit', 'limit', $app->get('list_limit'), 'int');
		$limitstart		  = $app->getUserStateFromRequest('com_jblance.limitstart', 'limitstart', 0, 'int');
		$filter_order     = $app->getUserStateFromRequest('com_jblance_filter_order_planbenefits', 'filter_order', 'l.lft', 'cmd');
		$filter_order_Dir = $app->getUserStateFromRequest('com_jblance_filter_order_Dir_planbenefits', 'filter_order_Dir', 'asc', 'word');
		$search			  = $app->getUserStateFromRequest('com_jblance_planbenefits_search', 'search', '', 'string');
		if(strpos($search, '"') !== false){
			$search = str_replace(array('=', '<'), '', $search);
		}
		$search = JString::strtolower($search);
		...
		$query = "SELECT * FROM #__jblance_planbenefits l".
				 $where.$orderby;
		$db->setQuery($query);
		$db->execute();//echo $query;
		$total = $db->getNumRows();
	...
	function getPlanBenefitAccess($id)
	{
    ...
    $db=JFactory::getDbo();
	$query="SELECT * FROM #__jblance_benefitaccess WHERE ben_id=".$id. " AND plan_id=".$cid[0];
	$db->setQuery($query);
	$result=$db->loadObject();
	$value = $result->type=="checkmark" ? $result-> state : $result->custom;
	return $value;
	...
	$db=JFactory::getDbo();
	$row=array();
	$query="SELECT * FROM #__jblance_planbenefits WHERE id!=1 AND parent_id=1 ORDER BY lft ASC";
	$db->setQuery($query);
	$rowsP=$db->loadObjectList();
	$parentchild=array();
	$parents=array();
	foreach($rowsP as $value)
	{
	$query="SELECT * FROM #__jblance_planbenefits WHERE id!=1 AND parent_id=".$value->id."  ORDER BY lft ASC";
	$db->setQuery($query);
	$rowsPC=$db->loadObjectList();
	$parentchild[$value->id]=$rowsPC;
	}
