...
class JblanceHelper {
   ...
   static function createThumbs($src_file, $thumb, $width = 200, $height = 200, $overwrite = true) {
...
//return true if the plan id is set 
    ...
    public static function checkOwnershipOfOperation($id, $type) {
        ...
        elseif ($type == 'projectprogress') {
            $query = "SELECT b.user_id assigned,p.publisher_userid publisher FROM #__jblance_bid b " .
                    ...
                    "WHERE p.id = " . $db->quote($id)." AND b.`status`='COM_JBLANCE_ACCEPTED'";
            $db->setQuery($query);
    ...
    public static function getBid($pid) {
        $user = JFactory::getUser();
        $db = JFactory::getDbo();
        $query = 'SELECT COUNT(*) FROM #__jblance_bid WHERE project_id=' .$pid. ' AND user_id = ' .$user->id;
        $db->setQuery($query);
...
// start upload file 				
				...
				WHERE vvm.`virtuemart_vendor_id`='" . $vendor_id . "' ";
                    ...
                    $profileurl = JPATH_BASE . '/images/jblance/';
                    if ($imgisvalid) {
                        //JFile::upload($image['tmp_name'], $profileurl.$newpicname);
                ...
                // end upload file 	
                //if(!JFile::upload($image['tmp_name'], $profileurl.$newpicname))
                $response['resufffflt'] = $profileurl . $newpicname;
                // JFile::upload($newpicname, );
                $response['result'] = 'OK';
                ...
                // $response['target_imagepath']=$target_imagepath;
                echo json_encode($response);
                exit;
            }
        }
    ...
    //3.Upload Photo
    public static function uploadProfilePictureMedia() {
        $app = JFactory::getApplication();
        $lang = JFactory::getLanguage();
        $lang->load('com_jblance', JPATH_SITE);
...
// start upload file 				
                ...
                if ($image['name'] != '') {
                    ...
                    $file_url_thumb = $vendorimages[2];
                    $vendorimage_path = 'images/jblance/';
                    $vendorthumb_path = 'images/jblance/';
                    $infosImg = getimagesize($image['tmp_name']);
                    //if ( (substr($image['type'],0,5) != 'image' || $infosImg[0] > $maximgside || $infosImg[1] > $maximgside ) ){
                    if ((substr($image['type'], 0, 5) != 'image')) {
                        JError::raiseWarning(100, JText::_('COM_VMVENDOR_VMVENADD_IMGEXTNOT'));
...
//                        $db = JFactory::getDBO();
//                        $q = "SELECT  * FROM `#__jblance_user` WHERE `user_id` = '" . $userid . "' ";
//                        $db->setQuery($q);
//                        $userrow = $db->loadRow();
                        $q = "UPDATE `#__jblance_user` SET 
						 `picture`='" . $db->escape($newpicname) . "' , 
						 `thumb`='" . $newtmbname . "'  
