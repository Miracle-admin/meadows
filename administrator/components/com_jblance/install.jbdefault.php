...
function defaultLocation(){
	$db = JFactory::getDbo();
	$query = "SELECT COUNT(*) FROM #__jblance_location WHERE extension=''";
	$db->setQuery($query);
	if(!$db->loadResult()){
		$query = "
		INSERT INTO `#__jblance_location` VALUES 
		('1', 'ROOT', '0', '0', '15', '0', '1', '', '', '', 'system', '0'),
		('2', 'India', '1', '1', '2', '1', '1', '{\"latitude\":\"0\",\"longitude\":\"0\"}', '', '', '', '0'),
		('3', 'France', '1', '3', '4', '1', '1', '{\"latitude\":\"0\",\"longitude\":\"0\"}', '', '', '', '0'),
		('4', 'Germany', '1', '5', '6', '1', '1', '{\"latitude\":\"0\",\"longitude\":\"0\"}', '', '', '', '0'),
		('5', 'Malaysia', '1', '7', '8', '1', '1', '{\"latitude\":\"0\",\"longitude\":\"0\"}', '', '', '', '0'),
		('6', 'Singapore', '1', '9', '10', '1', '1', '{\"latitude\":\"0\",\"longitude\":\"0\"}', '', '', '', '0'),
		('7', 'United Kingdom', '1', '11', '12', '1', '1', '{\"latitude\":\"0\",\"longitude\":\"0\"}', '', '', '', '0'),
		('8', 'United States', '1', '13', '14', '1', '1', '{\"latitude\":\"0\",\"longitude\":\"0\"}', '', '', '', '0');
		";
		$db->setQuery($query);
		$db->execute();
	}
...
function upgrade129_130(){
	...
	$registry = new JRegistry;
	$registry->loadString($config->params);
	if(!$registry->get('reviewMessages')){
	...
	foreach($groups as $group){
		...
		if(!$registry->get('allowAddPortfolio')){
...
function upgrade1500_1501(){
	...
	$registry = new JRegistry;
	$registry->loadString($config->params);
	if(!$registry->get('minServiceBasePrice')){
	...
	foreach($plans as $plan){
		...
		if(!$registry->get('flChargePerService')){
