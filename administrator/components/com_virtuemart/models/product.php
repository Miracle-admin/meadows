...
class VirtueMartModelProduct extends VmModel {
    ...
    function sortSearchListQuery($onlyPublished = TRUE, $virtuemart_category_id = FALSE, $group = FALSE, $nbrReturnProducts = FALSE, $langFields = array()) {
        ...
        VmModel::setPaginationLimits();
         $joinPrice = TRUE;
        if (!empty($this->keyword) and $this->keyword !== '') {
...
// 		vmdebug('my $this->searchcustoms ',$this->searchcustoms);
        ...
        // echo strpos($virtuemart_category_id, ','); die;
            $joinCategory = TRUE;
            if (strpos($virtuemart_category_id, ',') !== false) {
                   // $a = explode(",",$virtuemart_category_id); 
//                    foreach ($a as $a1){
//                        if($a1)
...
// 			$joinLang = false;
        ...
        if (!empty($onlyPublished) and $isSite) {
            $where[] = ' p.`published`="1" ';
            $pr = JRequest::getVar("product_price");
           // echo $pr; die;
             if ($pr!='') {
                 $pr1 = explode(",", $pr);
                  $where[] = ' pp.`product_price` >= "'.$pr1[0].'" ';
                  $where[] = ' pp.`product_price` <= "'.$pr1[1].'" ';
             }
            
//            if ($pr!='' && $pr == "b200") {
//                $where[] = ' pp.`product_price` <= "200" ';
//            } else if ($pr!='' && $pr == "a200b500") {
//                $where[] = ' pp.`product_price` <= "500" ';
//                $where[] = ' pp.`product_price` >= "200" ';
//            }elseif($pr!='' && $pr == "a500"){
//                 $where[] = ' pp.`product_price` >= "500" ';
//            }
    ...
    public function setFilter() {
        ...
        if (!$app->isSite()) { //persisted filter only in admin
            ...
            $this->virtuemart_category_id = vRequest::getVar('virtuemart_category_id', FALSE);
        }
    }
